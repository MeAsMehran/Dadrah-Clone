...
from ..core.models import NormalUserFunctions , LawyerFunctions
# Create your models here.


# managements methods will get a query set from the users based on their user type
...
class Users(AbstractUser):
    ...
    class NormalUser(AbstractUser):
        ...
        base_role = "NormalUser"     # for specifying the normal users and lawyers (specifying the user type)
    ...
    class Lawyer(AbstractUser):
        ...
        base_role = "Lawyer"        # for specifying the normal users and lawyers (specifying the user type)

        name = models.CharField(max_length=200, )
        gender = models.CharField(max_length=6, blank=False)
        phone = models.CharField(max_length=10, unique=True, help_text="9xx xxx xxxx", blank=False, null=False, error_messages={'unique': 'The phone number is already in use.'}, )
        is_verified = models.BooleanField(default=False, blank=False, help_text="Designates whether this lawyer has verified phone", )
        license_number = models.CharField(max_length=5, )                   # shomare parvande
        history_of_legal_license = models.CharField(max_length=50, blank=False)                                      # دارای سابقه پروانه وکالت
        ...
        skills = models.ForeignKey(LawyerFunctions.Skills, on_delete=models.CASCADE)
